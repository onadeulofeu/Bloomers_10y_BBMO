...
euk_raw_rel_tax_rep_f <- euk_raw_rel_tax_rep |>
  ...
  dplyr::select(date, year, month, day, relative_abundance, asv_num, group, supergroup, sample_id, reads, seq)
...
euk_all_nano <- euk_all |>
  dplyr::filter(fraction == 'nano') |>
  group_by(date) |>
  dplyr::filter(
      case_when( #when there's a _N_ that was chosen I want that, but when there's not then I'll keep the raw one _nano_
        any(str_detect(sample_id, '_N_')) ~ TRUE,
        any(str_detect(sample_id, '_N_')) == FALSE ~ any(str_detect(sample_id, '_nano_'))
        #str_detect(sample_id, '_nano_') ~ TRUE,
...
##check if I have more than one sample for the same date (STILL TO BE DONE) there's a part of the code that tryes to deal with this but is not working...
test <- euk_all_nano |>
  dplyr::select(date, sample_id) |>
...
## I try the deconstant function with pseudoconunt and observe what do we get (go to line 881)----
library(vegan)
asv_tab_10y_nano_clr <- decostand(euk_all_w_nano, method = 'clr', pseudocount = 1) |>
  rownames_to_column(var = 'sample_id') |>
  pivot_longer(cols = starts_with(c('A','C','G','T')), names_to = 'seq', values_to = 'clr')

asv_tab_10y_pico_clr <- decostand(euk_all_w_pico, method = 'clr', pseudocount = 1)|>
  rownames_to_column(var = 'sample_id') |>
  pivot_longer(cols = starts_with(c('A','C','G','T')), names_to = 'seq', values_to = 'clr')
