...
#write.csv(abund_asv_bloom_events_02, 'data/abund_asv_bloom_events_02.csv')
...
#write.csv(abund_asv_bloo_fl_09_13, 'data/abund_asv_bloo_fl_09_13.csv')
...
asv_tab_all_bloo_z_tax |>
  ...
  geom_point(data = asv_tab_bloo_pseudo_anom |>
             ...
             aes(date, abundance_value, color =  '#9F0011', alpha = 1), size = 1)+
  theme_bw()+
  guides(guide_legend(colour = 'none',
                      alpha = 'none'))+
  theme(panel.grid = element_blank(), text = element_text(size = 7), panel.border = element_blank(),
...
zclr_bloo_fl_09_13 <- asv_tab_all_bloo_z_tax |>
  dplyr::filter(fraction == '0.2') |>
  dplyr::filter(year %in% c('2009', '2010', '2011', '2012', '2013')) |>
  dplyr::filter(asv_num %in% abund_asv_bloom_events_02_unique$asv_num) |>
...
zscores_gen_ra_fl_09_13 <- zclr_bloo_fl_09_13 |>
  left_join(genes_counts_z_scores_f, by = c('date' = 'date.y')) 
...
genes_counts_z_scores_f_sim <- genes_counts_z_scores_f |>
  dplyr::select(date.y, z_score_gen, annot) |>
  rename(date = date.y, z_score = z_score_gen, variable = annot)
